...
Public Class BRA3510F
    ...
    Private Sub btnEdit_Click(sender As Object, e As EventArgs) Handles btnEdit.Click
        Dim xlForm As BRA3520X = Nothing

        Try

            ' REV_001↓
            'xlForm = New BRA3520X(Me)
            xlForm = New BRA3520X(Me, cboVerKubun.SelectedValue.ToString)
            ' REV_001↑
    ...
    Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click
        Try
            ...
            '個別結果検討表作成論理取込クラス生成
            ' REV_001↓
            'Using ExcelImport = New ImportKobetsuKekkaKentohyoSakuseiRonri()
            Using ExcelImport = New ImportKobetsuKekkaKentohyoSakuseiRonri(cboVerKubun.SelectedValue.ToString)
                ' REV_001↑
                '処理実行
    ...
    Private Sub btnOutPut_Click(sender As Object, e As EventArgs) Handles btnOutPut.Click
        Try
            ...
            Dim fileName As String = ComConst.個別結果検討表作成論理.出力用ファイル名称.reportName & "_" _
            ...
            Try
                ...
                ' REV_001↓
                'Using ExcelOutput = New BRA3510R(filePath)
                Using ExcelOutput = New BRA3510R(filePath, cboVerKubun.SelectedValue.ToString)
                    ' REV_001↑
    ...
    Public Sub UpdateDate(txt As TextBox, cbo As ComboBox)
        ...
        Using db As New DBAccess(ComUtil.GetCurrentConnectionString(CommonInfo.Koutei))
            ' REV_001↓
            'dtn = DAOOther.GetKobetsuKekkaKentohyoSakuseiRonriUpdateDate(db)
            dtn = DAOOther.GetKobetsuKekkaKentohyoSakuseiRonriUpdateDate(db, cbo.SelectedValue.ToString)
            ' REV_001↑
        End Using
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Class ImportKobetsuKekkaKentohyoSakuseiRonri
        ...
        Private Function Import(xlSheets As Excel.Sheets, myParent As Form) As Boolean
            ...
            'シートデータ取得
            Dim lstDc As List(Of Dictionary(Of String, String)) = ComUtil.KobetsuKekkaKentohyoSakuseiRonri.GetSheetData(xlSheets, CType(Me, ComObjectProcess))

            'エラーチェック
            Dim details As New List(Of String)
            ' REV_001↓
            'If Not ComUtil.KobetsuKekkaKentohyoSakuseiRonri.CheckError(lstDc, details) Then
            If Not ComUtil.KobetsuKekkaKentohyoSakuseiRonri.CheckError(lstDc, details, _versionKbn) Then
                ' REV_001↑
                '進捗ダイアログを閉じる
                ProgressDialog.endDispose()
                'メッセージフォーム表示
            ...
            Using db As New DBAccess(ComUtil.GetCurrentConnectionString(CommonInfo.Koutei))
                Try
                    ...
                    '個別結果検討表作成論理削除
                    ' REV_001↓
                    'DAOOther.DeleteKobetsuKekkaKentohyoSakuseiRonri(db)
                    DAOOther.DeleteKobetsuKekkaKentohyoSakuseiRonri(db, _versionKbn)
                    ' REV_001↑

                    For Each dc As Dictionary(Of String, String) In lstDc
                        '個別結果検討表作成論理追加
                        ' REV_001↓
                        'DAOOther.InsertKobetsuKekkaKentohyoSakuseiRonri(db, dc)
                        DAOOther.InsertKobetsuKekkaKentohyoSakuseiRonri(db, _versionKbn, dc)
                        ' REV_001↑
        ...
        Private Function CheckFileFormat(xlSheets As Excel.Sheets) As Boolean
            ...
            Try
                ...
                For Each wsh As Excel.Worksheet In xlSheets
                    Try
                        If wsh.Name.Equals(ComConst.個別結果検討表作成論理.出力用ファイル名称.SheetName) Then
                            bln = True
                            Exit For
                        End If
                    Finally
