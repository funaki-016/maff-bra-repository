...
Public Class BRA5310R
    ...
    Private Const HEADER_TITLE As String = "集計結果表作成論理入力・修正"
    ...
    Public Sub New(ByVal outPath As String, versionKbn As String, einouKeieitai As String)
        ' REV_001↑
        MyBase.New(ComConst.集計結果表作成論理.出力用ファイル名称.tempFileName, True, False, ComConst.集計結果表作成論理.出力用ファイル名称.reportName, outPath, True)
    ...
    Private Sub SetReportFormat(xlSheets As Excel.Sheets, dataCount As Integer)
        ...
        Try
            ...
            '行削除
            If dataCount < ComConst.集計結果表作成論理.出力用ファイル名称.Row.Max Then
                Me.DeleteRow(xlSheet, dataCount + ComConst.集計結果表作成論理.出力用ファイル名称.Row.First, ComConst.集計結果表作成論理.出力用ファイル名称.Row.Last)
            End If
