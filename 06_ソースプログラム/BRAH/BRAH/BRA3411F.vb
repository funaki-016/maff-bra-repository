...
''' </summary>
''' <remarks></remarks>
Public Class BRA3411F

    Private Sub BRA3411F_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            ' REV_001↓
            'バージョン区分コンボボックス設定
    ...
    Private Sub btnEdit_Click(sender As Object, e As EventArgs) Handles btnEdit.Click
        Dim xlForm As BRA3421X = Nothing

        Try

            ' REV_001↓
            'xlForm = New BRA3421X(Me)
            xlForm = New BRA3421X(Me, cboVerKubun.SelectedValue.ToString)
            ' REV_001↑
    ...
    Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click
        Try
            ...
            Dim ret As Boolean
            '個別結果表作成論理_営農個人取込クラス生成
            ' REV_001↓
            'Using ExcelImport = New ImportKobetsuKekkahyoSakuseiRonriEinouKobetsu()
            Using ExcelImport = New ImportKobetsuKekkahyoSakuseiRonriEinouKobetsu(cboVerKubun.SelectedValue.ToString)
                ' REV_001↑
                '処理実行
    ...
    Private Sub btnOutPut_Click(sender As Object, e As EventArgs) Handles btnOutPut.Click
        Try
            ...
            Dim fileName As String = ComConst.個別結果表作成論理_営農個人.出力用ファイル名称.reportName & "_営農個人.xlsx"
            ...
            Try
                ...
                ' REV_001↓
                'Using ExcelOutput = New BRA3411R(filePath)
                Using ExcelOutput = New BRA3411R(filePath, cboVerKubun.SelectedValue.ToString)
                    ' REV_001↑
    ...
    Public Sub UpdateDate(txt As TextBox, cbo As ComboBox)
        ...
        Using db As New DBAccess(ComUtil.GetCurrentConnectionString(CommonInfo.Koutei))
            ' REV_001↓
            'dtn = DAOOther.GetKobetsuKekkahyoSakuseiRonriEinouKobetsuUpdateDate(db)
            dtn = DAOOther.GetKobetsuKekkahyoSakuseiRonriEinouKobetsuUpdateDate(db, cbo.SelectedValue.ToString)
            ' REV_001↑
        End Using
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Class ImportKobetsuKekkahyoSakuseiRonriEinouKobetsu
        ...
        'Public Sub New()
        Public Sub New(versionKbn As String)
            ' REV_001↑
            MyBase.New(True)
        ...
        Private Function Import(xlSheets As Excel.Sheets, myParent As Form) As Boolean
            ...
            'シートデータ取得
            Dim lstDc As List(Of Dictionary(Of String, String)) = ComUtil.KobetsuKekkahyoSakuseiRonriEinouKobetsu.GetSheetData(xlSheets, CType(Me, ComObjectProcess))

            'エラーチェック
            Dim details As New List(Of String)
            ' REV_001↓
            'If Not ComUtil.KobetsuKekkahyoSakuseiRonriEinouKobetsu.CheckError(lstDc, details) Then
            If Not ComUtil.KobetsuKekkahyoSakuseiRonriEinouKobetsu.CheckError(lstDc, details, _versionKbn) Then
                '進捗ダイアログを閉じる
                ProgressDialog.endDispose()
                'メッセージフォーム表示
            ...
            Using db As New DBAccess(ComUtil.GetCurrentConnectionString(CommonInfo.Koutei))
                Try
                    ...
                    '個別結果表作成論理＿営農個人削除
                    ' REV_001↓
                    'DAOOther.DeleteKobetsuKekkahyoSakuseiRonriEinouKobetsu(db)
                    DAOOther.DeleteKobetsuKekkahyoSakuseiRonriEinouKobetsu(db, _versionKbn)
                    ' REV_001↑

                    For Each dc As Dictionary(Of String, String) In lstDc
                        '個別結果表作成論理＿営農個人追加
                        ' REV_001↓
                        'DAOOther.InsertKobetsuKekkahyoSakuseiRonriEinouKobetsu(db, dc)
                        DAOOther.InsertKobetsuKekkahyoSakuseiRonriEinouKobetsu(db, _versionKbn, dc)
                        ' REV_001↑
        ...
        Private Function CheckFileFormat(xlSheets As Excel.Sheets) As Boolean
            ...
            Try
                ...
                For Each wsh As Excel.Worksheet In xlSheets
                    Try
                        If wsh.Name.Equals(ComConst.個別結果表作成論理_営農個人.出力用ファイル名称.SheetName) Then
                            bln = True
                            Exit For
                        End If
                    Finally
