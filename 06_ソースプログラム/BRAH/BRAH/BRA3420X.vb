...
'//*************************************************************************************************
''' <summary>
''' 個別結果表作成論理入力・修正（EXCEL）
''' </summary>
''' <remarks></remarks>
Public Class BRA3420X
    ...
    ' REV_001↑
    ''' <summary>個別結果表作成論理管理画面</summary>
    Private _Form As BRA3410F
    ...
    'Public Sub New(ByRef frm As BRA3410F)
    Public Sub New(ByRef frm As BRA3410F, versionKbn As String)
        ' REV_001↑
        MyBase.New(frm, System.IO.Path.Combine(IniFileInfo.ExcelReportPath(), ComConst.個別結果表作成論理.入力用ファイル名称), True)
        Try
            _Form = frm
    ...
    Private Sub SetData()
        Try
            ...
            Using db As New DBAccess(ComUtil.GetCurrentConnectionString(CommonInfo.Koutei))
                '個別結果表作成論理取得
                ' REV_001↓
                'dt = DAOOther.GetKobetsuKekkahyoSakuseiRonri(db)
                dt = DAOOther.GetKobetsuKekkahyoSakuseiRonri(db, _versionKbn)
                ' REV_001↑
            End Using

            'シートデータ設定
            ComUtil.KobetsuKekkahyoSakuseiRonri.SetSheetData(dt, xlSheets, CType(Me, ExcelProcess))
        Catch ex As Exception
            Throw
        Finally
            'Excel後処理
