...
''' </summary>
''' <remarks></remarks>
Public Class BRA3510R

    Inherits ExcelOutputSingleBaseClass
    ''' <summary>ヘッダータイトル文字列</summary>
    Private Const HEADER_TITLE As String = "個別結果検討表作成論理入力・修正"
    ...
    Public Sub New(ByVal outPath As String, versionKbn As String)
        ' REV_001↑
        MyBase.New(ComConst.個別結果検討表作成論理.出力用ファイル名称.tempFileName, True, False, ComConst.個別結果検討表作成論理.出力用ファイル名称.reportName, outPath, True)
    ...
    Private Sub SetReportFormat(xlSheets As Excel.Sheets, dataCount As Integer)
        ...
        Try
            ...
            '行削除
            If dataCount < ComConst.個別結果検討表作成論理.出力用ファイル名称.Row.Max Then
                Me.DeleteRow(xlSheet, dataCount + ComConst.個別結果検討表作成論理.出力用ファイル名称.Row.First, ComConst.個別結果検討表作成論理.出力用ファイル名称.Row.Last)
            End If
