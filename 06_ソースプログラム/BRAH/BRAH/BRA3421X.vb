...
'//*************************************************************************************************
''' <summary>
''' 個別結果表作成論理入力・修正（EXCEL）（営農個人）
''' </summary>
''' <remarks></remarks>
Public Class BRA3421X
    ...
    Private Const HEADER_TITLE As String = "個別結果表作成論理入力・修正画面"
    ...
    ' REV_001↑
    ''' <summary>個別結果表作成論理管理（営農個人）画面</summary>
    Private _Form As BRA3411F
    ...
    ''' <remarks></remarks>
    'Public Sub New(ByRef frm As BRA3411F)
    Public Sub New(ByRef frm As BRA3411F, versionKbn As String)
        ' REV_001↑
        MyBase.New(frm, System.IO.Path.Combine(IniFileInfo.ExcelReportPath(), ComConst.個別結果表作成論理_営農個人.入力用ファイル名称), True)
        Try
            _Form = frm
    ...
    Private Sub SetData()
        Try
            ...
            Using db As New DBAccess(ComUtil.GetCurrentConnectionString(CommonInfo.Koutei))
                '個別結果表作成論理＿営農個人取得
                ' REV_001↓
                'dt = DAOOther.GetKobetsuKekkahyoSakuseiRonriEinouKobetsu(db)
                dt = DAOOther.GetKobetsuKekkahyoSakuseiRonriEinouKobetsu(db, _versionKbn)
                ' REV_001↑
            End Using

            'シートデータ設定
            ComUtil.KobetsuKekkahyoSakuseiRonriEinouKobetsu.SetSheetData(dt, xlSheets, CType(Me, ExcelProcess))
        Catch ex As Exception
            Throw
        Finally
            'Excel後処理
