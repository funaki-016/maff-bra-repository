...
'//*************************************************************************************************
''' <summary>
''' 個別結果検討表作成論理入力・修正（EXCEL）
''' </summary>
''' <remarks></remarks>
Public Class BRA3520X
    ...
    Private Const HEADER_TITLE As String = "個別結果検討表作成論理入力・修正画面"
    ...
    ' REV_001↑
    ''' <summary>個別結果検討表作成論理管理画面</summary>
    Private _Form As BRA3510F
    ...
    'Public Sub New(ByRef frm As BRA3510F)
    Public Sub New(ByRef frm As BRA3510F, versionKbn As String)
        ' REV_001↑
        MyBase.New(frm, System.IO.Path.Combine(IniFileInfo.ExcelReportPath(), ComConst.個別結果検討表作成論理.入力用ファイル名称), True)
        Try
            _Form = frm
    ...
    Private Sub SetData()
        Try
            ...
            Using db As New DBAccess(ComUtil.GetCurrentConnectionString(CommonInfo.Koutei))
                '個別結果検討表作成論理取得
                ' REV_001↓
                'dt = DAOOther.GetKobetsuKekkaKentohyoSakuseiRonri(db)
                dt = DAOOther.GetKobetsuKekkaKentohyoSakuseiRonri(db, _versionKbn)
                ' REV_001↑
            End Using

            'シートデータ設定
            ComUtil.KobetsuKekkaKentohyoSakuseiRonri.SetSheetData(dt, xlSheets, CType(Me, ExcelProcess))
        Catch ex As Exception
            Throw
        Finally
            'Excel後処理
