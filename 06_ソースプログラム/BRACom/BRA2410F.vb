...
''' </summary>
''' <remarks></remarks>
Public Class BRA2410F

    Private Sub BRA2410F_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            '更新日時設定
    ...
    Private Sub btnEdit_Click(sender As Object, e As EventArgs) Handles btnEdit.Click
        Dim xlForm As BRA2420X = Nothing

        Try

            xlForm = New BRA2420X(Me)
    ...
    Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click
        Try
            ...
            Dim ret As Boolean
            '調査票審査論理管理（範囲）取込クラス生成
            Using ExcelImport = New ImportChosahyoShinsaRonriRange()
                '処理実行
    ...
    Private Sub btnOutPut_Click(sender As Object, e As EventArgs) Handles btnOutPut.Click
        Try
            ...
            Dim fileName As String = ComConst.調査票審査論理範囲.出力用ファイル名称.reportName & "_" _
            ...
            Try
                ...
                Using ExcelOutput = New BRA2410R(filePath)
    ...
    Public Sub UpdateDate(txt As TextBox)
        ...
        Using db As New DBAccess(ComUtil.GetCurrentConnectionString(CommonInfo.Koutei))
            dtn = DAOOther.GetChosahyoShinsaRonriRangeUpdateDate(db)
        End Using
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Class ImportChosahyoShinsaRonriRange
        ...
        Private Function Import(xlSheets As Excel.Sheets, myParent As Form) As Boolean
            ...
            'シートデータ取得
            Dim lstDc As List(Of Dictionary(Of String, String)) = ComUtil.ChosahyoShinsaRonriRange.GetSheetData(xlSheets, CType(Me, ComObjectProcess))

            'エラーチェック
            Dim details As New List(Of String)
            If Not ComUtil.ChosahyoShinsaRonriRange.CheckError(lstDc, details) Then
                '進捗ダイアログを閉じる
                ProgressDialog.endDispose()
                'メッセージフォーム表示
            ...
            Using db As New DBAccess(ComUtil.GetCurrentConnectionString(CommonInfo.Koutei))
                Try
                    ...
                    '調査票審査論理範囲削除
                    DAOOther.DeleteChosahyoShinsaRonriRange(db)

                    For Each dc As Dictionary(Of String, String) In lstDc
                        '調査票審査論理範囲追加
                        DAOOther.InsertChosahyoShinsaRonriRange(db, dc)
        ...
        Private Function CheckFileFormat(xlSheets As Excel.Sheets) As Boolean
            ...
            Try
                ...
                For Each wsh As Excel.Worksheet In xlSheets
                    Try
                        If wsh.Name.Equals(ComConst.調査票審査論理範囲.出力用ファイル名称.SheetName) Then
                            bln = True
                            Exit For
                        End If
                    Finally
