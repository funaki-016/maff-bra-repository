...
'// -----------+------------+--------------------+-------------------------------------------------
'//  REV_001   | 2022.12.16 |Daiko               | 要件No4 バージョン区分追加
'//            |            |                    |
'//*************************************************************************************************
''' <summary>
''' 集計結果検討表出力
...
Public Class BRA9340R
    ...
    'Public Sub New(ByVal outDir As String, chosakubun As String, kentohyo As CreateSyukeiKentohyo)
    Public Sub New(ByVal outDir As String, chosakubun As String, kentohyo As CreateSyukeiKentohyo, ByVal chosaNen As String)
        'MyBase.New(ComConst.集計結果検討表.出力用ファイル名称.リスト(chosakubun).tempFileName, True, False, ComConst.集計結果検討表.出力用ファイル名称.リスト(chosakubun).reportName, outDir, True)
        MyBase.New(ComConst.集計結果検討表.出力用ファイル名称.リスト(Tuple.Create(chosakubun, ComUtil.getVersionKubunTaikei(chosaNen, chosakubun))).tempFileName, True, False, ComConst.集計結果検討表.出力用ファイル名称.リスト(Tuple.Create(chosakubun, ComUtil.getVersionKubunTaikei(chosaNen, chosakubun))).reportName, outDir, True)
        ' REV_001↑
    ...
    Protected Overrides Sub ReportEdit(Of T As Class)(xlSheets As Excel.Sheets, unit As T)
        ...
        ' REV_001↓
        'Dim fileName As String = ComConst.集計結果検討表.出力用ファイル名称.リスト(_chosakubun).reportName _
        '                         & "_" & pkeyThis.chosaNen & "_" & pkeyThis.syukeiNo & ".xlsx"
        Dim fileName As String = ComConst.集計結果検討表.出力用ファイル名称.リスト(Tuple.Create(_chosakubun, ComUtil.getVersionKubunTaikei(pkeyThis.chosaNen, _chosakubun))).reportName _
                                 & "_" & pkeyThis.chosaNen & "_" & pkeyThis.syukeiNo & ".xlsx"
        ' REV_001↑
        ...
        If dtSyukei(ComConst.集計結果表.テーブル名称(_chosakubun)(0)).Rows.Count > 0 Then
            ...
            ' REV_001↓
            'fileName = ComConst.集計結果検討表.出力用ファイル名称.リスト(_chosakubun).reportName _
            '           & "_" & dr("調査年").ToString _
            ...
            fileName = ComConst.集計結果検討表.出力用ファイル名称.リスト(Tuple.Create(_chosakubun, ComUtil.getVersionKubunTaikei(pkeyThis.chosaNen, _chosakubun))).reportName _
                       & "_" & dr("調査年").ToString _
                       & "_" & dr("集計番号").ToString _
                       & "_" & dr("生産費平均値種類").ToString _
                       & "_" & Integer.Parse(dr("地域コード").ToString).ToString("00") _
